...
{
    class CInstructionMov : CommunityToolkit.Mvvm.ComponentModel.ObservableObject, IInstruction
    {
        public CInstructionMov(List<Object> args, List<byte> bin)
        {
            OPCode = 0;
            Args = args;
            DisplayString = ("MOV [" + Args[0].ToString() + "] -> [" + Args[1].ToString() + "]");

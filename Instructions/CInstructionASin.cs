...
{
    class CInstructionASin: CommunityToolkit.Mvvm.ComponentModel.ObservableObject, IInstruction
    {
        public CInstructionASin(List<Object> args, List<byte> bin)
        {
            OPCode = 0x49;
            Args = args;
            DisplayString = ("ASIN [" + ((int)Args[0]).ToString("X2") + "], PUSH result; POP " + Args[1].ToString());

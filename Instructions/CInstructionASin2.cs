...
{
    class CInstructionASin2 : CommunityToolkit.Mvvm.ComponentModel.ObservableObject, IInstruction
    {
        public CInstructionASin2(List<Object> args, List<byte> bin)
        {
            OPCode = 0x4A;
            Args = args;
            DisplayString = ("ASIN [" + ((int)Args[0]).ToString("X2") + "] -> [" + Args[1].ToString() + "]; POP " + Args[2].ToString());

...
{
    class CInstructionAnd2 : CommunityToolkit.Mvvm.ComponentModel.ObservableObject, IInstruction
    {
        public CInstructionAnd2(List<Object> args, List<byte> bin)
        {
            OPCode = 0x36;
            Args = args;
            DisplayString = ("AND stack[" + Args[0].ToString() + "], stack[" + Args[1].ToString() + "] -> stack[" + Args[2].ToString() + "], POP " + Args[3].ToString());

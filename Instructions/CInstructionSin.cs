...
{
    class CInstructionSin: CommunityToolkit.Mvvm.ComponentModel.ObservableObject, IInstruction
    {
        public CInstructionSin(List<Object> args, List<byte> bin)
        {
            OPCode = 0x45;
            Args = args;
            DisplayString = ("SIN [" + ((int)Args[0]).ToString("X2") + "], PUSH result; POP " + Args[1].ToString());

...
{
    class CInstructionPushVec : CommunityToolkit.Mvvm.ComponentModel.ObservableObject, IInstruction
    {
        public CInstructionPushVec(List<Object> args, List<byte> bin)
        {
            OPCode = 0x16;
            Args = args;
            DisplayString = ("PUSH<coordinates> = (" + Args[0].ToString() + "; " + Args[1].ToString() + "; " + Args[2].ToString() + ")");

...
{
    class CInstructionNeq2 : CommunityToolkit.Mvvm.ComponentModel.ObservableObject, IInstruction
    {
        public CInstructionNeq2(List<Object> args, List<byte> bin)
        {
            OPCode = 0x3A;
            Args = args;
            DisplayString = ("NEQ [" + Args[0].ToString() + "], [" + Args[1].ToString() + "]  -> [" + Args[2].ToString() + "], POP " + Args[3].ToString());

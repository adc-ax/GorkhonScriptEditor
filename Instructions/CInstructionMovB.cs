...
{
    class CInstructionMovB : CommunityToolkit.Mvvm.ComponentModel.ObservableObject, IInstruction
    {
        public CInstructionMovB(List<Object> args, List<byte> bin)
        {
            OPCode = 1;
            Args = args;
            DisplayString = ("MOV<bool> " + ((Boolean)(Args[1])).ToString() + " -> [" + Args[0].ToString() + "]");

...
{
    class CInstructionMult2 : CommunityToolkit.Mvvm.ComponentModel.ObservableObject, IInstruction
    {
        public CInstructionMult2(List<Object> args, List<byte> bin)
        {
            OPCode = 0x33;
            Args = args;
            DisplayString = ("MULT stack[" + Args[0].ToString() + "], stack[" + Args[1].ToString() + "] -> stack[" + Args[2].ToString() + "], POP" + Args[3].ToString());

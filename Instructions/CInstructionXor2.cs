...
{
    class CInstructionXor2 : CommunityToolkit.Mvvm.ComponentModel.ObservableObject, IInstruction
    {
        public CInstructionXor2(List<Object> args, List<byte> bin)
        {
            OPCode = 0x38;
            Args = args;
            DisplayString = ("XOR stack[" + Args[0].ToString() + "] with stack[" + Args[1].ToString() + "] -> stack[" + Args[2].ToString() + "], POP " + Args[3].ToString());

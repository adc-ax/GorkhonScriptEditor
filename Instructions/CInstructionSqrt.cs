...
{
    class CInstructionSqrt : CommunityToolkit.Mvvm.ComponentModel.ObservableObject, IInstruction
    {
        public CInstructionSqrt(List<Object> args, List<byte> bin)
        {
            OPCode = 0x44;
            Args = args;
            DisplayString = ("SQRT [" + ((int)Args[0]).ToString("X2") + "], PUSH result; POP " + Args[1].ToString());

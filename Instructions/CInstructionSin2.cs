...
{
    class CInstructionSin2 : CommunityToolkit.Mvvm.ComponentModel.ObservableObject, IInstruction
    {
        public CInstructionSin2(List<Object> args, List<byte> bin)
        {
            OPCode = 0x46;
            Args = args;
            DisplayString = ("SIN [" + ((int)Args[0]).ToString("X2") + "] -> [" + Args[1].ToString() + "]; POP " + Args[2].ToString());

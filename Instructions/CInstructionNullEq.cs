...
{
    class CInstructionNullEq : CommunityToolkit.Mvvm.ComponentModel.ObservableObject, IInstruction
    {
        public CInstructionNullEq(List<Object> args, List<byte> bin)
        {
            OPCode = 0x2D;
            Args = args;
            DisplayString = ("NULLEQ stack[" + Args[0].ToString() + "] == NULLptr, PUSH, POP" + Args[1].ToString() + " vars from stack");

...
{
    class CInstructionNullNeq2 : CommunityToolkit.Mvvm.ComponentModel.ObservableObject, IInstruction
    {
        public CInstructionNullNeq2(List<Object> args, List<byte> bin)
        {
            OPCode = 0x40;
            Args = args;
            DisplayString = ("NULLNEQ2 stack[" + ((int)Args[0]).ToString("X2") + "] != NULLptr -> stack[" + Args[1].ToString() + "], POP " + Args[2].ToString());

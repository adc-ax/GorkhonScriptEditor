...
{
    class CInstructionXor : CommunityToolkit.Mvvm.ComponentModel.ObservableObject, IInstruction
    {
        public CInstructionXor(List<Object> args, List<byte> bin)
        {
            OPCode = 0x26;
            Args = args;
            DisplayString = ("XOR stack[" + ((int)Args[0]).ToString("X2") + "], stack[" + Args[1].ToString() + "], PUSH result, POP " + Args[2].ToString());

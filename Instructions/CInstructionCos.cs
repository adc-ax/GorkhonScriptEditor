...
{
    class CInstructionCos: CommunityToolkit.Mvvm.ComponentModel.ObservableObject, IInstruction
    {
        public CInstructionCos(List<Object> args, List<byte> bin)
        {
            OPCode = 0x47;
            Args = args;
            DisplayString = ("COS [" + ((int)Args[0]).ToString("X2") + "], PUSH result; POP " + Args[1].ToString());

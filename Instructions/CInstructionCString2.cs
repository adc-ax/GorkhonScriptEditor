...
{
    class CInstructionCString2 : CommunityToolkit.Mvvm.ComponentModel.ObservableObject, IInstruction
    {
        public CInstructionCString2(List<Object> args, List<byte> bin)
        {
            OPCode = 0x4E;
            Args = args;
            DisplayString = ("CSTRING [" + ((int)Args[0]).ToString("X2") + "] -> [" + Args[1].ToString() + "]; POP " + Args[2].ToString());

...
{
    class CInstructionCos2 : CommunityToolkit.Mvvm.ComponentModel.ObservableObject, IInstruction
    {
        public CInstructionCos2(List<Object> args, List<byte> bin)
        {
            OPCode = 0x48;
            Args = args;
            DisplayString = ("COS [" + ((int)Args[0]).ToString("X2") + "] -> [" + Args[1].ToString() + "]; POP " + Args[2].ToString());
